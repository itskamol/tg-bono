...
export class EmailSettingsScene {
  ...
  async onText(@Ctx() ctx: Context, @Message('text') text: string) {
    ...
    if (!sceneState.port) {
      ...
      await ctx.reply(`Port set to ${port}. Now, please enter the username (sending email address).`);
      return;
    }
    ...
    if (!sceneState.pass) {
      ...
      await ctx.reply(`Password has been received. Finally, please enter the recipient's email address.`);
      return;
    }
...
export class ScheduleSettingsScene {
  ...
  async onText(@Ctx() ctx: Context, @Message('text') text: string) {
    ...
    if (!sceneState.cron) {
      ...
      if (cronParts.length !== 5) {
        await ctx.reply(
          'Invalid cron string. It must have 5 parts. Please try again.',
        );
        return;
      }
  ...
  async onEnableDisable(@Ctx() ctx: Context) {
    ...
    await this.prisma.setting.upsert({
      ...
      create: { key: 'schedule_config', value: JSON.stringify(scheduleConfig) },
    });
  ...
  async onText(@Ctx() ctx: Context, @Message('text') text: string) {
    ...
    if (!sceneState.serviceAccountJson) {
      try {
        ...
        await ctx.reply(
          'Google Sheets export settings have been saved successfully!',
        );
        await ctx.scene.leave();
      } catch (e) {
        await ctx.reply(
